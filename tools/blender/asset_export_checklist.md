...
| 2 | Confirm from manifest: width=`____` mm, depth=`____` mm, height=`____` mm, `visual.glb_path`=`assets/____.glb` | ☐ |
...
| 9 | Bounding box equals the generated fixture's to the mm (step 8 in `EXPORT_REAL_ASSET.md`) | ☐ |
| 10 | `check_asset_ready.py --manifest … --glb /tmp/<module>.glb` → `RESULT: READY` | ☐ |
| 11 | **Do not** hand-edit the GLB binary; **do not** rely on a downstream tool to "fix" scale or axis | ☐ |
| 12 | Replace the committed fixture only after a separate fixture-swap PR adds the test update + sign-off marker | ☐ |
